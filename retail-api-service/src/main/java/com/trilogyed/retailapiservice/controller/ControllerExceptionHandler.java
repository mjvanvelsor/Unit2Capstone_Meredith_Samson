...
public class ControllerExceptionHandler {
   ...
   public ResponseEntity<VndErrors> insufficientInventoryException(InsufficientInventoryException e, WebRequest request) {
      VndErrors error = new VndErrors(request.toString(), e.getMessage());
      ResponseEntity<VndErrors> responseEntity = new ResponseEntity<>(error, HttpStatus.CONTINUE);
      return responseEntity;
   }
   
   @ExceptionHandler(value = {ServiceFailException.class})
   @ResponseStatus(HttpStatus.SERVICE_UNAVAILABLE)
   public ResponseEntity<VndErrors> serviceFailException(InsufficientInventoryException e, WebRequest request) {
      VndErrors error = new VndErrors(request.toString(), e.getMessage());
